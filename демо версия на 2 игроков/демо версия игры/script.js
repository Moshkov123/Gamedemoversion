...
const normallyButton= document.getElementById("normally");
const notnormalButton= document.getElementById("notnormal");
const normallyresetButton= document.getElementById("normallyreset");
const notnormalresetButton= document.getElementById("notnormalreset");
...
function Truck() {
	...
	const cabin = new THREE.Mesh(new THREE.BoxBufferGeometry(25, 30, 30), [
		...
		new THREE.MeshLambertMaterial({ color }), // назад
		new THREE.MeshLambertMaterial({ color, map: truckLeftTexture }),
		new THREE.MeshLambertMaterial({ color, map: truckRightTexture }),
		new THREE.MeshLambertMaterial({ color }), // верх top
		new THREE.MeshLambertMaterial({ color }) // bottom дно
	]);
	cabin.position.x = 40;
	cabin.position.z = 20;
	cabin.castShadow = true;
...
normallyButton.addEventListener("mouseup", function () {
	startGame();
	circle=4; 
	point=1
});
notnormalButton.addEventListener("mouseup", function () {
	startGame();
	circle=2; 
	point=1.5
});
normallyresetButton.addEventListener("mouseup", function () {
	reset();
	circle=4; 
	point=1
});
victorynormallyresetButton.addEventListener("mouseup", function () {
	reset();
	circle=4; 
	point=1
});
notnormalresetButton.addEventListener("mouseup", function () {
	reset();
	circle=2; 
	point=1.5
});
difficultresetButton.addEventListener("mouseup", function () {
	reset();
	circle=2; 
	point=1.5
});
...
normallyButton.addEventListener("touchstart", function () {
	startGame();
	circle=4;
	point=1
});
notnormalButton.addEventListener("touchstart", function () {
	startGame();
	circle=2;
	point=1.5
});
normallyresetButton.addEventListener("touchstart", function () {
	reset();
	circle=4;
	point=1
});
victorynormallyresetButton.addEventListener("touchstart", function () {
	reset();
	circle=4;
	point=1
});
notnormalresetButton.addEventListener("touchstart", function () {
	reset();
	circle=2;
	point=1.5
});
difficultresetButton.addEventListener("touchstart", function () {
	reset();
	circle=2;
	point=1.5
});
...
function animation(timestamp) {
	...
	if (otherVehicles.length < (laps + 1) / circle) {
		addVehicle();
		car+=1
	}
...
function getVehicleSpeed(type) {
	if (type == "car") {
		const minimumSpeed = 1.2;
		const maximumSpeed = 2.2;
